...
import { listUserWorkspaces, listWatchlists } from "@/lib/persistence/watchlist-repository";
...
export async function CompanyWatchDock({ siren }: { siren: string }) {
  ...
  if (isAuthConfigured()) {
    try {
      ...
      // Optional authenticated context must never make the public company page unavailable.
    }
  }
  ...
  if (userId) {
    try {
      ...
      // The public intelligence page remains usable when workspace reads are unavailable.
    }
  }
  ...
  return (
    <aside className={styles.dock} aria-label="Mise sous veille de l’entreprise">
      ...
      {!userId ? (
        <Link className={styles.primaryLink} href={`/auth/sign-in?returnTo=${encodeURIComponent(returnTo)}`}>

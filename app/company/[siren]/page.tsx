...
export default async function CompanyPage({ params }: { params: Promise<{ siren: string }> }) {
  ...
  return (
    <AppShell>
      <div className="workspace company-workspace intelligence-v04">
        ...
        <section className="intelligence-score-strip" aria-label="Indicateurs Intelligence V0.4">
                ...
                <div className="legal-event-list">
                  {enrichment.legalEvents.slice(0, 12).map((event) => (
                    <div className={`legal-event ${event.risk}`} key={event.id}>
                      ...
                      <div><strong>{event.family}</strong>{event.description ? <p>{event.description}</p> : null}<small>{event.city || "BODACC / DILA"}</small></div>
        ...
        <section className="v04-section" id="organisation">
          ...
          <div className="company-grid-v04">
            ...
            <article className="detail-panel span-two">
              <div className="panel-title-row"><h3>Établissements</h3><span>{company.establishments.length} détaillé(s) / {company.openEstablishmentCount ?? "?"} ouvert(s)</span></div>
              <div className="establishment-grid-v04">
                {company.establishments.length ? company.establishments.slice(0, 30).map((establishment, index) => (
                  <div key={`${establishment.siret || establishment.address}-${index}`}>
                    ...
                    <small>{establishment.siret ? `SIRET ${establishment.siret}` : "SIRET non disponible"}{establishment.headOffice ? " · siège" : ""}</small>
                  </div>
                )) : <p>Aucun établissement détaillé retourné.</p>}
              </div>
              {(company.openEstablishmentCount || 0) > company.establishments.length ? <p className="data-caveat">La source consolidée indique davantage d’établissements ouverts que de fiches détaillées renvoyées dans cette réponse. Le moteur n’invente pas les établissements manquants.</p> : null}
            </article>
          </div>
        </section>

        <section className="v04-section" id="digital">
          <div className="v04-section-heading"><span>05</span><div><h2>Digital</h2><p>Présence web recoupée et empreinte technologique.</p></div></div>
          <article className="detail-panel web-intelligence-v04">
            <div className="panel-title-row"><h3><Globe2 size={16} /> Présence web</h3><span>{web?.domain ? "résolue" : "partielle"}</span></div>
            {web ? (
              <>
                <div className="web-domain-v04"><div><strong>{web.domain || "Domaine non résolu"}</strong><p>{officialActivity || "Activité officielle non libellée"}</p>{web.description ? <small>Description fournisseur web à recouper : {web.description}</small> : null}</div>{web.websiteUrl ? <a href={web.websiteUrl} target="_blank" rel="noreferrer"><ExternalLink size={16} /></a> : null}</div>
                <div className="web-kpis">
                  <div><span>SERP</span><strong>{web.serpPosition ? `#${web.serpPosition}` : "—"}</strong></div>
                  <div><span>Emails génériques</span><strong>{web.genericEmails.length}</strong></div>
                  <div><span>Téléphones publics</span><strong>{web.phoneNumbers.length}</strong></div>
                  <div><span>Technologies</span><strong>{web.technologies.length}</strong></div>
                </div>
        ...
        <section className="v04-section" id="contacts">
          <div className="v04-section-heading"><span>06</span><div><h2>Contacts</h2><p>Enrichissement professionnel uniquement à la demande.</p></div></div>
          <article className="detail-panel"><ContactReveal siren={company.siren} domain={web?.domain} /></article>
        </section>
        ...
        <section className="v04-section" id="timeline">
          ...
          <div className="timeline-v04">
            ...
            {enrichment.legalEvents.slice(0, 15).map((event) => (
              <div key={`bodacc-${event.id}`}><span>{formatFrenchDate(event.date)}</span><i className={event.risk} /><div><strong>{event.family}</strong>{event.description ? <p>{event.description}</p> : null}<small>BODACC / DILA</small></div></div>
            ))}
